# Omperator release gate

Omperator is a client for a separate runtime. A green UI fixture suite proves that the renderer understands its fixtures; it does not prove that a released desktop build can discover OMP, load real history, survive a reconnect, or work through the Tailnet gateway.
...
9. On macOS, download the public DMG through a browser, copy Omperator to Applications, and open it without a quarantine-removal command. Confirm Gatekeeper reports the pinned Developer ID publisher and launches the app normally.
