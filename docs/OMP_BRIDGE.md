...
Omperator v0.2.1 is paired with immutable OMP tag `t4code-17.0.5-appserver-20` at commit `7b8eb298b83fc85c36eb278c4ced2d249deaae17`. Its published Apple Silicon binary size and SHA-256 are pinned in `compat/omp-app-matrix.json`.
...
That bridge release moves the running network host into the standalone T4 executable, removes OMP's public legacy launchers, lets copied sessions target an existing working directory when the original directory is gone, and records a durable authority marker in new transcripts so a completed short terminal session can be reclaimed safely. The sparse bridge projects that marker to external hosts only after verifying the bounded transcript header on disk. The thin bridge and standalone host pass a compiled-binary end-to-end smoke test. The compatibility matrix records `appserver-20` as both the verified and published pairing for the T4 product build.
