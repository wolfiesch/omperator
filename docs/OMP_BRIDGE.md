...
Omperator v0.2.1 is paired with immutable OMP tag `t4code-17.0.5-appserver-18` at commit `ccb86bb74f4c59ebacca67d42c4e16b65fd5c8ec`. Its published Apple Silicon binary is 120,975,568 bytes with SHA-256 `0d7e27c8c8bdbdc261cc6f886b8184f53ba08842a81c3e2eee22dabd940de011`.
...
That bridge release moves the running network host into the standalone T4 executable, removes OMP's public legacy launchers, lets copied sessions target an existing working directory when the original directory is gone, and records a durable authority marker in new transcripts so a completed short terminal session can be reclaimed safely. The thin bridge and standalone host pass a compiled-binary end-to-end smoke test. The compatibility matrix records `appserver-18` as both the verified and published pairing for the T4 product build.
