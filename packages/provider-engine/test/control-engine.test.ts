...
import { encodeProviderRequest,type ProviderMethod,type WireValue } from "../src/machine-provider-v1.js";
...
test("open_machine bounds sleeping wake and reports retryable phase timeout",async()=>{
 ...
 const opened=response((await waking.session.receive(request("open","open_machine",{machine_id:"runtime",workspace_mirror_authority:false})))[0]!);
