...
export class TermScreen {
  ...
  private cx = 0;
  private cy = 0;
  private static readonly MAX_CURSOR_COLUMN = 4096;
  private static readonly MAX_CURSOR_ROW = 4000;

  feed(text: string): void {
    // Strip OSC and charset/mode sequences we don't model.
    /* eslint-disable no-control-regex -- stripping real ANSI escapes is the point. */
    const s = text
      .replace(/\u001b\][^\u0007\u001b]*(?:\u0007|\u001b\\)/g, "")
      .replace(/\u001b[()][A-Z0-9]/gi, "")
      .replace(/\u001b[>=]/g, "");
    ...
    for (let i = 0; i < s.length; i += 1) {
      ...
      if (ch === "\u001b") {
        ...
        if (m) {
          ...
          switch (m[2]) {
            case "A":
              this.cy = Math.max(0, this.cy - (n || 1));
              break;
            case "B":
              this.cy += n || 1;
              break;
            case "C":
              ...
              break;
            case "D":
              this.cx = Math.max(0, this.cx - (n || 1));
              break;
            case "G":
              this.cx = Math.max(0, n - 1);
              break;
            case "d":
              this.cy = Math.max(0, n - 1);
              break;
            case "H":
            case "f":
            ...
            case "J":
              if (n === 0) {
                this.setLine(this.cy, this.line().slice(0, this.cx));
                this.lines.length = this.cy + 1;
              } else if (n === 2) {
                this.lines = [];
                this.cy = 0;
                this.cx = 0;
          ...
          this.cx = Math.min(TermScreen.MAX_CURSOR_COLUMN, this.cx);
          this.cy = Math.min(TermScreen.MAX_CURSOR_ROW, this.cy);
          i += m[0].length - 1;
          continue;
        }
        i += 1;
      ...
      if (ch === "\n" || ch === "\x0b" || ch === "\f") {
        this.cy = Math.min(TermScreen.MAX_CURSOR_ROW, this.cy + 1);
        this.cx = 0;
        continue;
      }
      if (ch === "\b") {
      ...
      if (ch === "\t") {
        this.cx = Math.min(TermScreen.MAX_CURSOR_COLUMN, (this.cx + 8) & ~7);
        continue;
      }
      ...
      this.cx = Math.min(TermScreen.MAX_CURSOR_COLUMN, this.cx + 1);
    }
    if (this.lines.length > 2000) {
      const removed = this.lines.length - 2000;
      this.lines.splice(0, removed);
      this.cy = Math.max(0, this.cy - removed);
    }
  }
...
export class Tui implements HostEvents {
  private state: State = {
    ...
    diffLines: [],
    diffScroll: 0,
    diffLoadedFor: undefined,
    terminalId: undefined,
    termOpenedFor: undefined,
    termScroll: 0,
  };
  ...
  private feedInput(data: string): void {
    ...
    for (let i = 0; i < this.inputBuf.length; i += 1) {
      ...
      /* eslint-disable no-control-regex -- escape-sequence framing needs real ANSI escapes. */
      if (
        /^\u001b\[[<0-9;?]*[A-Za-z~]/.test(rest) ||
        /^\u001b[()][A-Z0-9]/i.test(rest) ||
        /^\u001b[>=]/.test(rest)
      )
        continue;
  ...
  private railRows(): RailRow[] {
    ...
    s.sessions.forEach((sess, index) => {
      if (
        ...
        !(sess.project?.name ?? "").toLowerCase().includes(q)
      )
        return;
  ...
  private async loadDiff(): Promise<void> {
    ...
    try {
      ...
      this.state.diffLines = [
        `files.diff failed: ${error instanceof Error ? error.message : error}`,
      ];
    }
    this.draw();
  }
  ...
  private termRows(): number {
    return Math.max(4, this.screen.rows - 5);
  }
  private async ensureTerminal(): Promise<void> {
    ...
    try {
      ...
      this.termScreen.lines = [
        `term.open failed: ${error instanceof Error ? error.message : error}`,
      ];
    }
    this.draw();
  }
  ...
  private input(data: string): void {
    for (let i = 0; i < data.length; i += 1) {
      ...
      if (ch === "\u001b") {
        ...
        else if (data.slice(i, i + 4) === "\u001b[5~") {
          this.key("pageup");
          i += 1;
        } else if (data.slice(i, i + 4) === "\u001b[6~") {
          this.key("pagedown");
          i += 1;
  ...
  private mouse(btn: number, x: number, y: number, down: boolean): void {
    ...
    if (btn === 64 || btn === 65) {
      ...
      if (s.pane === "chat") s.scroll = Math.max(0, Math.min(this.maxScroll(), s.scroll + delta));
        ...
        if (s.fileView)
          s.fileView.scroll = Math.max(
            0,
            Math.min(Math.max(0, s.fileView.lines.length - 1), s.fileView.scroll + delta),
          );
        else
          s.fileScroll = Math.max(
            0,
            Math.min(Math.max(0, s.fileEntries.length - 1), s.fileScroll + delta),
          );
      } else if (s.pane === "search")
        s.searchScroll = Math.max(
          0,
          Math.min(Math.max(0, s.searchResults.length - 1), s.searchScroll + delta),
        );
      else if (s.pane === "diff")
        s.diffScroll = Math.max(
          0,
          Math.min(Math.max(0, s.diffLines.length - 1), s.diffScroll + delta),
        );
      else if (s.pane === "term")
        s.termScroll = Math.max(
          0,
          Math.min(Math.max(0, this.termScreen.lines.length - 1), s.termScroll + delta),
        );
      this.draw();
      return;
    }
  ...
  private key(name: string): void {
    ...
    switch (name) {
      ...
      case "down":
        ...
        break;
      case "pageup":
        s.scroll = Math.min(this.maxScroll(), s.scroll + this.bodyRows() - 4);
        break;
      case "pagedown":
        s.scroll = Math.max(0, s.scroll - (this.bodyRows() - 4));
        break;
      case "left":
        if (s.focus === "composer") s.cursorIdx = Math.max(0, s.cursorIdx - 1);
        break;
      case "right":
        if (s.focus === "composer") s.cursorIdx = Math.min(s.composer.length, s.cursorIdx + 1);
        break;
      case "home":
        if (s.focus === "composer") s.cursorIdx = 0;
        break;
      case "end":
        if (s.focus === "composer") s.cursorIdx = s.composer.length;
        break;
      case "tab":
        s.focus = s.focus === "rail" ? "composer" : "rail";
        break;
      case "escape":
        if (s.focus === "composer" && s.composer) {
          s.composer = "";
          s.cursorIdx = 0;
        } else s.focus = "rail";
        break;
  ...
  private filesKey(name: string): void {
    ...
    if (s.fileView) {
      switch (name) {
        case "up":
          s.fileView.scroll = Math.max(0, s.fileView.scroll - 1);
          break;
        case "down":
          s.fileView.scroll = Math.min(
            Math.max(0, s.fileView.lines.length - 1),
            s.fileView.scroll + 1,
          );
          break;
        case "pageup":
          s.fileView.scroll = Math.max(0, s.fileView.scroll - (this.bodyRows() - 2));
          break;
        case "pagedown":
          s.fileView.scroll = Math.min(
            Math.max(0, s.fileView.lines.length - 1),
            s.fileView.scroll + this.bodyRows() - 2,
          );
          break;
        case "escape":
        case "backspace":
          s.fileView = undefined;
          break;
      }
      this.draw();
      return;
    ...
    switch (name) {
      case "up":
        s.fileSelected = Math.max(0, s.fileSelected - 1);
        break;
      case "down":
        s.fileSelected = Math.min(s.fileEntries.length - 1, s.fileSelected + 1);
        break;
      case "pageup":
        s.fileSelected = Math.max(0, s.fileSelected - (this.bodyRows() - 2));
        break;
      case "pagedown":
        s.fileSelected = Math.min(s.fileEntries.length - 1, s.fileSelected + this.bodyRows() - 2);
        break;
      case "backspace": {
        if (!s.filePath) break;
  ...
  private searchKey(name: string): void {
    ...
    switch (name) {
      case "up":
        s.searchSelected = Math.max(0, s.searchSelected - 1);
        break;
      case "down":
        s.searchSelected = Math.min(s.searchResults.length - 1, s.searchSelected + 1);
        break;
      case "backspace":
  ...
  private diffKey(name: string): void {
    ...
    switch (name) {
      case "up":
        s.diffScroll = Math.max(0, s.diffScroll - 1);
        break;
      case "down":
        s.diffScroll = Math.min(Math.max(0, s.diffLines.length - 1), s.diffScroll + 1);
        break;
      case "pageup":
        s.diffScroll = Math.max(0, s.diffScroll - (this.bodyRows() - 2));
        break;
      case "pagedown":
        s.diffScroll = Math.min(
          Math.max(0, s.diffLines.length - 1),
          s.diffScroll + this.bodyRows() - 2,
        );
        break;
      case "escape":
        this.setPane("chat");
  ...
  private termKey(name: string): void {
    ...
    switch (name) {
      case "escape":
        ...
        return;
      case "up":
        s.termScroll = Math.min(Math.max(0, this.termScreen.lines.length - 1), s.termScroll + 1);
        break;
      case "down":
        s.termScroll = Math.max(0, s.termScroll - 1);
        break;
      case "pageup":
        s.termScroll = Math.min(
          Math.max(0, this.termScreen.lines.length - 1),
          s.termScroll + this.bodyRows() - 2,
        );
        break;
      case "pagedown":
        s.termScroll = Math.max(0, s.termScroll - (this.bodyRows() - 2));
        break;
      case "enter":
        this.client.termInput(id, tid, "\r");
        break;
      case "backspace":
        this.client.termInput(id, tid, "\x7f");
        break;
      case "left":
        this.client.termInput(id, tid, "\u001b[D");
        break;
      case "right":
        this.client.termInput(id, tid, "\u001b[C");
        break;
      case "tab":
        this.client.termInput(id, tid, "\t");
        break;
      default:
        return;
    }
  ...
  private drawChat(): void {
    ...
    if (s.scroll === 0 && s.entries.length > 0) {
      ...
      if (last.length > bodyRows && transcript.length > last.length)
        tSlice = transcript.slice(
          transcript.length - last.length,
          transcript.length - last.length + bodyRows,
        );
    }
  ...
  private entryLines(e: TranscriptEntry, width: number): Cell[][] {
    ...
    if (role === "user" || kind === "user" || kind === "prompt") {
      label = "you";
      color = p.gold;
    } else if (role === "assistant" || kind === "message") {
      label = "agent";
      color = p.foam;
    } else if (kind === "tool-use" || kind === "tool" || kind === "tool_call") {
      label = "tool";
      color = p.pine;
    } else if (kind === "tool-result" || kind === "tool_result") {
      label = "result";
      color = p.iris;
    } else if (kind === "turn-review") {
      label = "review";
      color = p.iris;
    } else if (kind === "error") {
      label = "error";
    ...
    if (headline && headline !== label) {
      for (const line of wrap(headline, width - 2))
        lines.push([{ text: `  ${line}`, fg: p.ink, bold: true }]);
    }
    if (body) {
      const dim = kind === "compaction" || /think/i.test(kind);
      for (const line of wrap(body, width - 2))
        lines.push([{ text: `  ${line}`, fg: dim ? p.muted : p.body, dim }]);
    }
  ...
  private drawFiles(): void {
    ...
    this.screen.line([
      { text: ` /${s.filePath}`, fg: p.accent, bold: true },
      { text: `  ${s.fileEntries.length} entries`, fg: p.label, dim: true },
    ]);
    ...
    for (let i = 0; i < bodyRows - 1; i += 1) {
      ...
      this.screen.line([
        ...
        {
          text: clip(entry.path, this.screen.cols - 18),
          fg: isSel ? p.ink : entry.kind === "directory" ? p.pine : p.body,
          bold: isSel,
        },
    ...
    this.screen.line([
      ...
      { text: clip(s.searchQuery, this.screen.cols - 12), fg: p.ink },
      {
        text: s.searching
          ? "  searching…"
          : s.searchQuery
            ? `  ${s.searchResults.length} matches`
  ...
  private drawFooter(): void {
    ...
    if (s.pane === "chat") {
      ...
      this.screen.line([
        {
          text: focused ? label : `  ${label.trim()} `,
          fg: focused ? p.accent : p.label,
          bold: focused,
        },
        ...(s.composer
          ? focused
