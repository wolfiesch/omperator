...
//  Authoritative session inventory for the native T4 Code Linux client: the
//  real host-wire SessionRef inventory, grouped by project, drives the rail
//  from a HostClient connection. Restores the saved host-wire endpoint on
//  launch; connect(endpoint:) swaps in a real t4-host. Seeded with sample data
//  so the rail renders without a live host.
...
final class T4SessionStore: ObservableObject {
    ...
    /// Direct-host credentials remain available to the retained engine, but
    /// the current ordinary onboarding surface uses account + public relay.
    ...
    var hostId: String = ""
    /// Rendezvous host connected through the public relay. The saved pair
    /// link is the reconnect credential and remains scoped to this host id.
    ...
    func restore() async {
        ...
        guard !connected, !connecting, credentialPersistenceEnabled else { return }
        do {
    ...
    private func persist(
        ...
        do {
            ...
            savedHosts.insert(
                WindowsSavedHostSummary(id: credential.id, endpoint: credential.displayEndpoint),
                at: 0
            )
            return nil
        } catch {
    ...
    init(
        ...
        if credentialPersistenceEnabled, !launchArguments.contains("-T4Demo") {
            do {
                savedHosts = try self.savedHostCredentialStore.allCredentials().map {
                    WindowsSavedHostSummary(id: $0.id, endpoint: $0.displayEndpoint)
                }
            } catch {
    ...
    private func makeTransport(endpoint: URL) -> any HostWireTransport {
        makePlatformHostWireTransport(endpoint: endpoint)
    }
    ...
    /// Connect through the current public relay model. A fresh pairing code
    /// is redeemed exactly once; later opens rejoin with the saved pair link.
    func connectPublicHost(hostId: String, code: String?, name: String) async {
        ...
        let client = HostClient(transport: transport, config: HostClient.Config(
            identity: ClientIdentity(
                name: name,
                version: "0.2",
                build: "dev",
        ...
        self.client = client
        await client.setOnReconnected { [weak self] in
            Task { await self?.handleTransportReconnected() }
        }
        do {
            let welcome = try await client.connect()
            self.hostId = welcome.hostId
            hostInfo = HostInfo(
                hostId: welcome.hostId,
                ompVersion: welcome.ompVersion,
                appserverVersion: welcome.appserverVersion
            )
            grantedCapabilities = welcome.grantedCapabilities
            grantedFeatures = Set(welcome.grantedFeatures)
            connected = welcome.authentication == .paired || welcome.authentication == .local
            guard connected else {
                ...
                lastError = "The computer did not accept this pairing."
                return
            }
            ...
            Keychain.remove(forKey: Self.relayLinkKey(hostId: hostId))
            connectedRelayHostId = nil
            lastError = "Connection interrupted. Try again in a moment."
        }
    }

    private func redeemPairCode(hostId: String, code: String) async -> String? {
        var request = URLRequest(
            url: T4DiscoveryClient.rendezvousBaseURL().appendingPathComponent("v1/pair")
        )
        ...
        request.httpBody = try? JSONSerialization.data(
            withJSONObject: ["hostId": hostId, "code": code]
        )
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let http = response as? HTTPURLResponse, http.statusCode == 200,
                  let body = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                  let link = body["pairLink"] as? String,
                  !link.isEmpty
            else {
                lastError = "Pairing code rejected. Check the code and try again."
                return nil
            }
            return link
        } catch {
            lastError = "Can't reach the pairing service. Check your connection and try again."
            return nil
        }
    }
    ...
    func disconnect() async {
        ...
        // Direct-host records remain in Credential Manager until the retained
        // saved-host API explicitly forgets one.
