...
enum WindowsSecretStoreError: Error, Equatable, LocalizedError {
    case invalidKey
    case invalidValue
    case payloadTooLarge
    case malformedCredential
    case credentialManager(operation: WindowsSavedHostCredentialError.Operation, code: UInt32)

    var errorDescription: String? {
        switch self {
        case .invalidKey:
            return "The credential key is invalid."
        case .invalidValue:
            return "The credential value is invalid."
        case .payloadTooLarge:
            return "The credential is too large for Windows Credential Manager."
        case .malformedCredential:
            return "A saved Omperator credential is damaged or unsupported."
        case .credentialManager(let operation, let code):
            return "Windows Credential Manager could not \(operation.rawValue) Omperator credentials (error \(code))."
        }
    }
}

protocol WindowsSecretStoring: AnyObject, Sendable {
    func value(forKey key: String) throws -> String?
    func set(_ value: String, forKey key: String) throws
    func remove(forKey key: String) throws
}

final class WindowsInMemorySecretStore: WindowsSecretStoring, @unchecked Sendable {
    private let lock = NSLock()
...
/// Versioned key/value credentials used by the current account, public-relay,
/// and single-host restore model. Target names contain only a namespace and
/// SHA-256 key digest; both the logical key and secret value live in the
/// Credential Manager blob.
final class WindowsCredentialManagerSecretStore: WindowsSecretStoring, @unchecked Sendable {
    static let productionNamespace = "net.t4code.app/Omperator/Secret/v2"

    private static let credentialType = DWORD(CRED_TYPE_GENERIC)
    private static let persistence = DWORD(CRED_PERSIST_LOCAL_MACHINE)
    private static let maximumBlobSize = Int(CRED_MAX_CREDENTIAL_BLOB_SIZE)
    private static let magic = Array("OMPSECRET2".utf8)
    private static let maximumKeyBytes = 512
    ...
    func value(forKey key: String) throws -> String? {
        let targetName = try targetName(for: key)
        var target = wideString(targetName)
        var pointer: PCREDENTIALW?
        let found = target.withUnsafeMutableBufferPointer { buffer in
            CredReadW(buffer.baseAddress, Self.credentialType, 0, &pointer)
        }
        guard found else {
            let code = GetLastError()
            if code == DWORD(ERROR_NOT_FOUND) { return nil }
            throw WindowsSecretStoreError.credentialManager(operation: .read, code: code)
        }
        guard let pointer else {
            throw WindowsSecretStoreError.malformedCredential
        }
        defer { CredFree(pointer) }
        guard pointer.pointee.Type == Self.credentialType else {
            throw WindowsSecretStoreError.malformedCredential
        }
        let decoded = try Self.decode(blob(from: pointer.pointee))
        guard decoded.key == key else {
            throw WindowsSecretStoreError.malformedCredential
        }
    ...
    func set(_ value: String, forKey key: String) throws {
        let targetName = try targetName(for: key)
        var target = wideString(targetName)
        var blob = try Self.encode(key: key, value: value)
        let written = target.withUnsafeMutableBufferPointer { targetBuffer in
            blob.withUnsafeMutableBytes { blobBuffer in
                var credential = CREDENTIALW()
                credential.Flags = 0
                credential.Type = Self.credentialType
                credential.TargetName = targetBuffer.baseAddress
                credential.Comment = nil
                credential.CredentialBlobSize = DWORD(blobBuffer.count)
                credential.CredentialBlob = blobBuffer.baseAddress?.assumingMemoryBound(to: BYTE.self)
                credential.Persist = Self.persistence
                credential.AttributeCount = 0
                credential.Attributes = nil
                credential.TargetAlias = nil
                credential.UserName = nil
                return CredWriteW(&credential, 0)
            }
        }
        guard written else {
            throw WindowsSecretStoreError.credentialManager(operation: .save, code: GetLastError())
        }
    }
    ...
    func remove(forKey key: String) throws {
        let targetName = try targetName(for: key)
        var target = wideString(targetName)
        let removed = target.withUnsafeMutableBufferPointer { buffer in
            CredDeleteW(buffer.baseAddress, Self.credentialType, 0)
        }
        guard removed else {
            let code = GetLastError()
            if code == DWORD(ERROR_NOT_FOUND) { return }
            throw WindowsSecretStoreError.credentialManager(operation: .delete, code: code)
        }
    }

    private func targetName(for key: String) throws -> String {
        guard !key.isEmpty,
              key.utf8.count <= Self.maximumKeyBytes,
              key.unicodeScalars.allSatisfy({ !CharacterSet.controlCharacters.contains($0) })
        else {
            throw WindowsSecretStoreError.invalidKey
        }
    ...
    private static func encode(key: String, value: String) throws -> Data {
        guard !value.isEmpty,
              value.unicodeScalars.allSatisfy({ !CharacterSet.controlCharacters.contains($0) })
        else {
            throw WindowsSecretStoreError.invalidValue
        }
        let fields = [key, value]
        let byteCount = fields.reduce(into: magic.count + fields.count * 4) {
            $0 += $1.utf8.count
        }
        guard byteCount <= maximumBlobSize else {
            throw WindowsSecretStoreError.payloadTooLarge
        }
    ...
    private static func decode(_ data: Data) throws -> (key: String, value: String) {
        let bytes = [UInt8](data)
        guard bytes.count >= magic.count + 8,
              Array(bytes.prefix(magic.count)) == magic
        else {
            throw WindowsSecretStoreError.malformedCredential
        }
        ...
        guard offset == bytes.count, !fields[0].isEmpty, !fields[1].isEmpty else {
            throw WindowsSecretStoreError.malformedCredential
        }
...
enum WindowsSecretStoreFactory {
    static func make(
        arguments: [String] = ProcessInfo.processInfo.arguments
    ) -> any WindowsSecretStoring {
        if WindowsCredentialAccessPolicy.allowsPersistentCredentials(arguments: arguments) {
            return WindowsCredentialManagerSecretStore()
        }
        return WindowsInMemorySecretStore()
    }
}
