...
struct T4SessionStoreFixtureIntegrationTests {
    ...
    func liveHostFlowAndReconnect() async throws {
        ...
        do {
            ...
            let initialResponseCount = store.transcript(for: session.sessionId)
            ...
            try await waitForStore("durable response replaced live streaming state") {
                store.streamingMessages[session.sessionId] == nil
                    && !store.activeTurns.contains(session.sessionId)
            }
            let responseCountBeforeDrop = store.transcript(for: session.sessionId)
                .count(where: { $0.body == "Hello world" })
            #expect(responseCountBeforeDrop == initialResponseCount + 1)
