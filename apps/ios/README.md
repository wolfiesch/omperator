...
| Transport security | **wss:// direct connects via `--remote-tls-port 8788` with TOFU cert pinning** | The wss listener authenticates the host against rogue network peers. The app pins the leaf SHA-256 in the Keychain on first connect (`T4CertPinner`) and fails closed if the pin cannot be persisted. ATS stays `NSAllowsArbitraryLoads` because the narrower WebSocket key was not honored in the tested iOS 26 environment. |

iOS **cannot bundle a host**; it connects to an **existing** `t4-host` via the
public relay + pairing code, exactly like the Android client.
...
ports, no VPN, works on any network. The rendezvous URL is baked as
