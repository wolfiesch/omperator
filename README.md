...
T4 owns the client wire, generic host service, and standalone daemon in `@t4-code/host-wire`, `@t4-code/host-service`, and `@t4-code/host-daemon`. The frozen `@oh-my-pi/app-wire` 0.7.0 tarball remains only as compatibility evidence. The released package still launches `t4-host` against the strict `t4-omp-authority/1` bridge, while OMP owns session files, locks, agent execution, and takeover decisions. Unmodified stock OMP is the supported official-authority runtime: the host probes `omp --version` and accepts the gate-proven builds (17.0.9, see `compat/official-omp-gate0.json`) silently, any version in the 17.x window with an advisory warning, and refuses only older runtimes or unknown majors. `OMP_T4_STRICT_RUNTIME=1` restores the byte-exact pin for packaged builds that ship a specific runtime. New stock versions enter the window as data — a row in the host's runtime matrix after the maintainer gates pass — not as host code changes. Packaged desktop and managed-session cutover proofs remain before the official path replaces the released bridge fallback.
