  ...
  const cases = [
    ...
    [
      ...
      (text) =>
        text.replace(
          "needs: [changes, t4-api-generation, check, unit-tests, build-e2e, m1-build-e2e, current-bridge-continuity, cluster, tooling, maintainer, android-debug]",
          "needs: [changes, t4-api-generation, check, unit-tests, build-e2e, m1-build-e2e, legacy-bridge-continuity, current-bridge-continuity, official-omp-gate0, cluster, tooling, maintainer, android-debug]",
        ),
    ],
    [
      ".github/workflows/ci.yml",
...
test("deploys release site source only after artifact publication", () => {
  ...
  assert.ok(ciWorkflow.includes("m1-build-e2e:"));
  assert.ok(ciWorkflow.includes("runs-on: [self-hosted, macOS, ARM64, wolfie-m1, trusted]"));
  assert.ok(ciWorkflow.includes("run: scripts/run-with-m1-lock.sh scripts/ci-m1-build-e2e.sh"));
  ...
  assert.ok(ciWorkflow.includes("M1_BUILD_E2E_RESULT: ${{ needs.m1-build-e2e.result }}"));
  assert.ok(ciWorkflow.includes('case "$BUILD_E2E_RESULT:$M1_BUILD_E2E_RESULT" in'));
